...
export default async function ContactPage() {
  ...
  const channels = [
    { icon: Mail, title: t("emailTitle"), desc: t("emailDesc"), color: "bg-primary-50 text-primary-600" },
    { icon: MessageCircle, title: t("messagingTitle"), desc: t("messagingDesc"), color: "bg-success-50 text-success-600" },
    { icon: Clock, title: t("responseTitle"), desc: t("responseDesc"), color: "bg-warning-50 text-warning-600" },
  ];
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <Card>
        <h2 className="mb-5 text-lg font-semibold text-gray-900">{t("formTitle")}</h2>
        <ContactForm />
      </Card>
    </div>
  );
